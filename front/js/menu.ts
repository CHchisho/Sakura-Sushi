...
export const initializeFilterButtons = (): void => {
  const menu_filters_buttons =
    document.querySelectorAll<HTMLButtonElement>('.menu_filters_button')
...
export const generateMenuItem = (item: MenuItem): string => {
    ...
    <div class="menu_item">
        <div class="top">
            <div class="top_info">
				...
				<p>Available days: ${item.availableDays
          .map((day) => DAYS_NAMES[day as keyof typeof DAYS_NAMES])
          .join(', ')}</p>
            </div>
  ...
  html += `<button class="button_add" data-item-id="${item.id}" aria-label="Add to cart"><i class="bi bi-plus-lg"></i></button>
        </div>
    </div>
    `
...
export const generateMenu = (
  ...
  const active_button = document.querySelector<HTMLButtonElement>(
    `.menu_filters_button[value="${filterType || 'All'}"]`
  )
  if (active_button) {
...
export const initializeMap = async (): Promise<void> => {
  ...
  try {
    const [restaurants, busStopsData] = await Promise.all([getRestaurants(), getBusStops()])
